...
class Loconet(nn.Module):
    ...
    def forward(self, audioFeature, visualFeature, landmark, labels, masks):
        ...
        if self.consistency_lambda > 0.0:
            ...
            audioEmbed, visualEmbed = self.model.forward_cross_attention(audioEmbed, visualEmbed)
            outsAV = self.model.forward_audio_visual_backend(audioEmbed, visualEmbed, b, s)
            outsAVNonLandmark = outsAV.clone()

            labels = labels.reshape((-1))
            masks = masks.reshape((-1))
            _, predScoresNonLandmark, _, _ = self.lossAV.forward(outsAV, labels, masks)

            if self.consistency_method == 'kl':
                ...
                consistency_loss = self.consistency_lambda * self.mse_loss(outsAVNonLandmark, outsAVLandmark.detach())
